...
class MobilityIndicator(Indicator):
    def setup(self,*args,**kwargs):
        self.fitted_model_object_loc='./tables/corktown/fitted_co2_model.p'
    ...
    def train(self):
        ...
        numerical_cols=[col for col in X_df.columns if not col=='future_mobility']
...
#        co2_model=lm.fit(X, y_co2)
    ...
    def normalised_prediction(self, model, X_in, y_max, y_min):
        y_pred=self.co2_model.predict(X_in)[0]
        print(y_pred)
    ...
    def load_module(self):
        print('loading')
        try:
            fitted_co2_model=pickle.load(open(self.fitted_model_object_loc, 'rb'))
    ...
    def return_indicator(self, geogrid_data):
        ...
        for feat in self.co2_model_features:
            if feat=='future_mobility':
                ...
                x=0               
            X.append(x)
        co2_norm=self.normalised_prediction(self.co2_model, np.array(X).reshape(1, -1), self.max_co2, self.min_co2)
        print(co2_norm)
        self.value_indicators=[{'name': 'Mobility CO2 Performance', 'value': 1-co2_norm, 
                 'viz_type': self.viz_type},
                {'name': 'Mobility Health Impacts', 'value': 1-co2_norm, 
                 'viz_type': self.viz_type}]
